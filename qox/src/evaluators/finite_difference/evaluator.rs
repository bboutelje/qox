...
use crate::{market::market_data::OptionMarketData, solvers::finite_difference::{meshing::{log::LogMesher1d, uniform::UniformMesher1d}, operators_log::{TridiagonalCoeffs, solve_tridiagonal}}, traits::{EvaluationResolver, fdm_1d_mesher::Mesher1d, instrument::OptionInstrument, pricing_engine::OptionEvaluable, rate_curve::RateCurve, real::Real, vol_surface::VolSurface}};
...
impl<I, SReal, RC, VS> OptionEvaluable<I, SReal, VS::T, <SReal as EvaluationResolver<RC::T, VS::T>>::Output, RC, VS> for Evaluator
where
    ...
    Neg<Output = <SReal as EvaluationResolver<RC::T, VS::T>>::Output>,
{
    ...
    fn evaluate(&self, instrument: &I, market: &OptionMarketData<SReal, RC, VS>) -> Self::Result
    {
        let s_min = Self::Result::from_f64(0.01);
        let spot = Self::Result::from_real(market.spot_price.clone());
        let s_max = &spot * &Self::Result::from_f64(5.0);
        ...
        // 2. PROMOTE them to T (the workhorse type)
        let r = Self::Result::from_real(r_raw);
        let sigma = Self::Result::from_real(sigma_raw);

        // 3. Pass the promoted T values to the operator

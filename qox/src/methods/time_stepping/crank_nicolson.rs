...
impl<T: Real> TimeStepper<T, 1, 2> for CrankNicolson<T> {
    ...
    fn prepare_stage_rhs(
        ...
        let y_n = &vector.items[0..n];
        let f_n = &vector.items[n..2 * n];
    ...
    fn finalize_step(&self, vector: &mut InputVector<T>, ws: &GlmWorkspace<T>, dt: T) {
        ...
        let (y_hist, f_hist) = vector.items.split_at_mut(n);
...
mod tests {
    use super::*;
    // Note: Ensure InputVector and GlmWorkspace are accessible or imported
    ...
    fn crank_nicolson_convergence_order() {
        ...
        for &n_steps in &steps {
            ...
            for _ in 0..n_steps {
                // Note: GLM step now updates vector.current_time internally
